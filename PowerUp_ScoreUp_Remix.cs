...
public class PowerUp_ScoreUp_Remix : MonoBehaviour
{
    ...
    {
        Enemy.scoreMultiplier = bonusMultiplier; // changes the score multiplier to the bonus value

        yield return new WaitForSeconds(length); // wait for a specified "length" of time (seconds)

        Enemy.scoreMultiplier = defaultMultiplier; // changes the score multiplier back to the default value
